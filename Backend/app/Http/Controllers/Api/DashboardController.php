...
class DashboardController extends Controller
{
    ...
    private function getStatsInternal(Request $request)
    {
        $viewType = $request->get('view_type', 'monthly');
        $userId = auth()->id();
        $cacheKey = "dashboard_stats_{$viewType}_{$userId}";
    ...
    private function getChartDataInternal(Request $request)
    {
        $viewType = $request->get('view_type', 'monthly');
        $cacheKey = 'dashboard_chart_' . $viewType . '_' . auth()->id();
    ...
    private function getSavingCompositionInternal(Request $request)
    {
        $userId = auth()->id();
        $isAdmin = auth()->user()->hasAnyRole(['admin', 'ketua', 'bendahara', 'sekretaris']);
        $cacheKey = 'dashboard_composition_' . ($isAdmin ? 'admin' : $userId);
    ...
    private function getRecentActivitiesInternal(Request $request)
    {
        $limit = 5;
        $cacheKey = 'dashboard_activities_' . auth()->id();
    ...
    private function getQuickLinksInternal(Request $request)
    {
        $user = $request->user();
        $links = [];
