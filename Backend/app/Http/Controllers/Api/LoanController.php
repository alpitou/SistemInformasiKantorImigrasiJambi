...
{
    const MAX_LOAN_AMOUNT = 10000000; // Rp 10.000.000
    const MIN_LOAN_AMOUNT = 100000;   // Rp 100.000
    const INTEREST_RATE = 1;          // 1% per bulan
    ...
    private function canApplyForLoan($userId, $newAmount)
    {
        ...
        if (!$activeLoan) {
            // No active loan, allowed
            return ['allowed' => true, 'message' => null];
        }

        // Calculate remaining percentage
        $totalLoan = $activeLoan->amount + ($activeLoan->amount * $activeLoan->interest_rate / 100);
        $paidAmount = $totalLoan - $activeLoan->remaining_balance;
        $paidPercentage = ($paidAmount / $totalLoan) * 100;
        ...
        if ($totalWithNew > self::MAX_LOAN_AMOUNT) {
            return [
                'allowed' => false,
                'message' => 'Total pinjaman (termasuk pengajuan ini) melebihi batas maksimal Rp ' . number_format(self::MAX_LOAN_AMOUNT, 0, ',', '.')
            ];
        }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $request->validate([
                'amount' => 'required|numeric|min:' . self::MIN_LOAN_AMOUNT . '|max:' . self::MAX_LOAN_AMOUNT,
            ...
            $interestRate = $request->interest_rate ?? self::INTEREST_RATE;

            // PERHITUNGAN BUNGA: 1% dari TOTAL PINJAMAN
    ...
    public function generateDraftAgreement(Request $request)
    {
        try {
            $request->validate([
                ...
                'tenor_months' => 'required|integer|min:3|max:10',
            ...
            $tempLoan->id = 'DRAFT';
            $tempLoan->amount = $request->amount;
            $tempLoan->interest_rate = $request->interest_rate;
            $tempLoan->tenor_months = $request->tenor_months;
            $tempLoan->monthly_installment = ($request->amount + ($request->amount * $request->interest_rate / 100)) / $request->tenor_months;
            $tempLoan->purpose = $request->purpose;

            $totalPayment = $request->amount + ($request->amount * $request->interest_rate / 100);
            $pokokPerBulan = $request->amount / $request->tenor_months;
            $bungaPerBulan = ($request->amount * $request->interest_rate / 100);

            $html = $this->generateAgreementHTML([
                'loan' => $tempLoan,
                'user' => $tempUser,
                'monthlyInstallment' => $tempLoan->monthly_installment,
    ...
    public function submitWithDocument(Request $request)
    {
        try {
            ...
            if ($activeLoan) {
                $totalLoanWithInterest = $activeLoan->amount + ($activeLoan->amount * $activeLoan->interest_rate / 100);
                ...
                if ($totalExistingLoans + $request->amount > 10000000) {
                    return response()->json([
                        'success' => false,
                        'message' => 'Total pinjaman (termasuk pengajuan ini) melebihi batas maksimal Rp 10.000.000'
                    ], 422);
                }
            }
    ...
    public function generateAgreement($id)
    {
        try {
            ...
            $bungaPerBulan = ($loan->amount * $loan->interest_rate / 100);
    ...
    /**
     * Generate Agreement HTML with stamp between date and signature
     */
    private function generateAgreementHTML($data)
    {
        ...
        <body>
            ...
            <div class="clause">
                Pada hari ini, <strong>' . date('l, d F Y') . '</strong>, kami yang bertanda tangan di bawah ini:
            </div>
            ...
            <div class="clause">
                Dengan ini mengajukan pinjaman kepada Koperasi Kantor Imigrasi Kelas I TPI Jambi, selanjutnya disebut sebagai <strong>PIHAK KEDUA / KOPERASI</strong>.
            </div>
            ...
            <div class="clause">
                Pinjaman ini diberikan untuk jangka waktu selama <strong>' . $loan->tenor_months . ' ( ' . $this->terbilang($loan->tenor_months) . ' ) bulan</strong>.
            </div>

            <div class="clause-title">Pasal 3: BUNGA DAN ANGSURAN</div>
            <div class="clause">
                Atas pinjaman ini dikenakan bunga sebesar <strong>' . $loan->interest_rate . '% ( ' . $this->terbilang($loan->interest_rate) . ' persen ) flat</strong> per bulan.
            </div>
            ...
            <table class="detail-table">
                ...
                <tbody>
                    <tr>
                        ...
                        <td class="text-right">Rp ' . number_format($pokokPerBulan, 0, ',', '.') . '</td>
                    ...
                    <tr>
                        ...
                        <td class="text-right">Rp ' . number_format($bungaPerBulan * $loan->tenor_months, 0, ',', '.') . '</td>
                    </tr>
                    <tr class="total-row">
            ...
            <div class="clause">
                PIHAK KESATU wajib membayar angsuran setiap bulan sebesar <strong>Rp ' . number_format($monthlyInstallment, 0, ',', '.') . '</strong> yang dibayarkan paling lambat setiap tanggal <strong>25</strong> setiap bulannya melalui pemotongan gaji.
            </div>

            <div class="clause-title">Pasal 4: KETENTUAN TOP UP PINJAMAN</div>
            <div class="clause">
                Peminjaman kembali (top up) dapat dilakukan apabila pinjaman sebelumnya telah dilunasi sedikitnya <strong>80% (delapan puluh persen)</strong> dari total pinjaman (termasuk bunga), dengan total pinjaman tidak melebihi batas maksimal peminjaman yakni <strong>Rp ' . number_format(self::MAX_LOAN_AMOUNT, 0, ',', '.') . '</strong>.
            </div>
            ...
            <!-- Tanda Tangan dengan Materai di antara Tanggal dan Nama -->
            <div class="signature-wrapper">
                <div class="signature-container">
                    <div class="signature-date">Jambi, ' . date('d F Y') . '</div>
                    
                    <!-- Materai di antara tanggal dan nama peminjam -->
                    <div class="stamp-box">
                        <span>Rp 10.000</span>
                        <small>(Materai)</small>
                    </div>
