...
const Settings: React.FC = () => {
    ...
    try {
      ...
      if (response.data.success && response.data.data) {
        ...
        setLoanSimulation(prev => ({ 
          ...prev, 
          tenor: response.data.data.max_tenor_months,
          amount: Math.min(prev.amount, response.data.data.max_loan_amount)
        }));
        updateLoanSimulation(response.data.data);
      ...
      if (savedSettings) {
        try {
          ...
          updateLoanSimulation(parsed);
        } catch (e) {
          console.error('Failed to parse saved loan settings:', e);
        }
      }
    }
  }, []);

  // Fetch general info from localStorage
  const fetchGeneralInfo = useCallback(() => {
    const savedInfo = localStorage.getItem('cooperative_settings');
    if (savedInfo) {
      try {
        const parsed = JSON.parse(savedInfo);
        setGeneralInfo(parsed);
      } catch (e) {
        console.error('Failed to parse saved general info:', e);
      }
    }
  }, []);
  ...
  useEffect(() => {
    ...
    fetchLoanSettings();
    fetchGeneralInfo();
  }, [fetchSavingTypes, fetchLoanSettings, fetchGeneralInfo]);
  ...
  const handleLoanAmountChange = (amount: number) => {
    const minAmount = loanSettings.min_loan_amount;
    const maxAmount = loanSettings.max_loan_amount;
    let validAmount = amount;
    if (validAmount < minAmount) validAmount = minAmount;
    if (validAmount > maxAmount) validAmount = maxAmount;
    
    const totalInterestAmount = (validAmount * loanSettings.default_interest_rate) / 100;
    const totalPayment = validAmount + totalInterestAmount;
  ...
  // Settings.tsx - setelah save berhasil
  const handleSaveLoanSettings = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        window.dispatchEvent(new CustomEvent('loanSettingsChanged', { 
          detail: loanSettings 
        }));
      ...
      addNotification({
        title: 'Berhasil (Lokal)',
        message: 'Pengaturan pinjaman disimpan secara lokal',
        type: 'success'
      });
    } finally {
      setIsLoading(false);
  ...
  const tabs = [
    { id: 'general', label: 'Umum', icon: Building, description: 'Informasi dasar koperasi' },
    { id: 'savings', label: 'Jenis Simpanan', icon: Wallet, description: 'Kelola jenis dan nominal simpanan' },
    { id: 'loan', label: 'Pinjaman & Bunga', icon: Percent, description: 'Konfigurasi parameter pinjaman' },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <SettingsIcon size={28} className="text-imigrasi-primary" />
            Pengaturan Sistem
          </h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            Konfigurasi operasional, parameter keuangan, dan pengaturan koperasi
          </p>
          <div className="mt-2 flex flex-wrap gap-2">
            <span className="bg-blue-100 text-blue-700 text-xs px-2 py-1 rounded-full">🏢 Informasi Koperasi</span>
            <span className="bg-blue-100 text-blue-700 text-xs px-2 py-1 rounded-full">💰 Jenis Simpanan</span>
            <span className="bg-blue-100 text-blue-700 text-xs px-2 py-1 rounded-full">📊 Pinjaman & Bunga</span>
          </div>
        </div>
        <button 
          onClick={() => { fetchSavingTypes(); fetchLoanSettings(); }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        {/* Sidebar Tabs */}
        <div className="bg-white dark:bg-neutral-800 rounded-2xl border border-gray-200 dark:border-neutral-700 p-4 h-fit shadow-sm">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider px-3 mb-3">Menu Pengaturan</p>
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={cn(
                  ...
                  activeTab === tab.id 
                    ? "bg-imigrasi-primary text-white shadow-md" 
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          {activeTab === 'general' && (
            <motion.div
              ...
              className="bg-white dark:bg-neutral-800 rounded-2xl border border-gray-200 dark:border-neutral-700 overflow-hidden shadow-sm"
            >
              <div className="px-6 py-5 border-b border-gray-200 dark:border-neutral-700 bg-gradient-to-r from-blue-50 to-white dark:from-blue-900/10 dark:to-transparent">
                <h3 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                  <Building size={22} className="text-imigrasi-primary" />
                  Informasi Koperasi
                </h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  Informasi dasar yang akan ditampilkan di berbagai dokumen dan laporan
                </p>
              </div>
              ...
              <div className="p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider flex items-center gap-2">
                      <Building size={14} /> Nama Koperasi
                    </label>
                    <input 
                      type="text" 
                      value={generalInfo.cooperative_name}
                      onChange={(e) => setGeneralInfo({...generalInfo, cooperative_name: e.target.value})}
                      className="w-full px-4 py-3 bg-gray-50 dark:bg-neutral-700 border border-gray-200 dark:border-neutral-600 rounded-xl focus:border-imigrasi-primary focus:ring-1 focus:ring-imigrasi-primary outline-none transition-all dark:text-white" 
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider flex items-center gap-2">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider flex items-center gap-2">
                    <FileText size={14} /> Alamat Kantor
                  </label>
                  <textarea 
                    rows={3} 
                    value={generalInfo.address}
                    onChange={(e) => setGeneralInfo({...generalInfo, address: e.target.value})}
                    className="w-full px-4 py-3 bg-gray-50 dark:bg-neutral-700 border border-gray-200 dark:border-neutral-600 rounded-xl focus:border-imigrasi-primary focus:ring-1 focus:ring-imigrasi-primary outline-none transition-all dark:text-white resize-none" 
                  />
                </div>
                ...
                <div className="flex justify-end pt-4 border-t border-gray-100 dark:border-neutral-700">
                  <button 
                    ...
                    disabled={isLoading}
                    className="flex items-center gap-2 px-6 py-2.5 bg-imigrasi-primary text-white rounded-xl text-sm font-bold hover:bg-blue-800 transition-colors shadow-md disabled:opacity-50"
                  >
            ...
            >
              <div className="px-6 py-5 border-b border-gray-200 dark:border-neutral-700 bg-gradient-to-r from-blue-50 to-white dark:from-blue-900/10 dark:to-transparent">
                <div className="flex flex-wrap justify-between items-center gap-4">
                  <div>
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                      <Wallet size={22} className="text-imigrasi-primary" />
                      Jenis Simpanan
                    </h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-imigrasi-primary text-white rounded-xl text-sm font-bold hover:bg-blue-800 transition-colors shadow-md"
                  >
                    <Plus size={16} />
                    Tambah Jenis
                  </button>
              ...
              <div className="p-6">
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                    <thead className="bg-gray-50 dark:bg-neutral-800/80 rounded-xl">
                      <tr className="text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wider">
                        <th className="px-4 py-3 font-semibold">Nama Simpanan</th>
                        <th className="px-4 py-3 font-semibold">Nominal Default</th>
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-neutral-700">
                      {savingTypes.map((type) => (
                        <tr key={type.id} className="hover:bg-gray-50 dark:hover:bg-neutral-700/30 transition-colors">
                          ...
                          <td className="px-4 py-4">
                            <div className="flex items-center gap-2">
                              <span className="font-bold text-imigrasi-primary">
          ...
          {activeTab === 'loan' && (
            ...
            >
              <div className="px-6 py-5 border-b border-gray-200 dark:border-neutral-700 bg-gradient-to-r from-blue-50 to-white dark:from-blue-900/10 dark:to-transparent">
                <h3 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                  <Percent size={22} className="text-imigrasi-primary" />
                  Pengaturan Pinjaman
                </h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              ...
              <div className="p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      onChange={(e) => {
                        const newTenor = Math.min(parseInt(e.target.value) || 10, 12);
                      ...
                      className="w-full px-4 py-3 bg-gray-50 dark:bg-neutral-700 border border-gray-200 dark:border-neutral-600 rounded-xl focus:border-imigrasi-primary focus:ring-1 focus:ring-imigrasi-primary outline-none transition-all dark:text-white"
                    />
                    <p className="text-xs text-gray-400 mt-1">Maksimal 12 bulan sesuai kebijakan koperasi</p>
                  </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider flex items-center gap-2">
                      <DollarSign size={12} /> Minimal Pinjaman (IDR)
                    </label>
                    <div className="relative">
                      ...
                      <input 
                        ...
                        min="50000"
                        step="50000"
                        className="w-full pl-12 pr-4 py-3 bg-gray-50 dark:bg-neutral-700 border border-gray-200 dark:border-neutral-600 rounded-xl focus:border-imigrasi-primary focus:ring-1 focus:ring-imigrasi-primary outline-none transition-all dark:text-white"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider flex items-center gap-2">
                      <DollarSign size={12} /> Maksimal Pinjaman (IDR)
                    </label>
                    <div className="relative">
                      <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm">Rp</span>
                      <input 
                        ...
                        onChange={(e) => setLoanSettings({...loanSettings, max_loan_amount: parseInt(e.target.value) || 10000000})}
                        min="1000000"
                        step="1000000"
                        className="w-full pl-12 pr-4 py-3 bg-gray-50 dark:bg-neutral-700 border border-gray-200 dark:border-neutral-600 rounded-xl focus:border-imigrasi-primary focus:ring-1 focus:ring-imigrasi-primary outline-none transition-all dark:text-white"
                      />
                ...
                <div className="p-5 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
                  <h4 className="font-bold text-blue-800 dark:text-blue-400 mb-4 flex items-center gap-2">
                    <Calculator size={18} /> Simulasi Pinjaman
                  </h4>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <div className="relative w-48">
                          ...
                          <input
                            ...
                            onChange={(e) => handleLoanAmountChange(parseInt(e.target.value) || 0)}
                            min={loanSettings.min_loan_amount}
                            max={loanSettings.max_loan_amount}
                            className="w-full pl-8 pr-2 py-1 text-right text-sm bg-white dark:bg-neutral-800 border border-blue-200 dark:border-blue-800 rounded-lg"
                          />
                        </div>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Tenor</span>
                        <span className="font-bold">{loanSimulation.tenor} bulan</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Bunga ({loanSettings.default_interest_rate}%)</span>
                        <span className="font-bold">{formatCurrency(loanSimulation.totalInterest)}</span>
                      </div>
                    </div>
                    <div className="border-t md:border-t-0 md:border-l border-blue-200 dark:border-blue-800 pt-3 md:pt-0 md:pl-4">
                      <div className="flex justify-between mb-2">
                        <span className="text-gray-600">Angsuran per bulan</span>
                        <span className="font-bold text-imigrasi-primary text-lg">{formatCurrency(loanSimulation.monthlyInstallment)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Total Pembayaran</span>
                        <span className="font-bold">{formatCurrency(loanSimulation.totalPayment)}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-xl flex gap-3">
                  ...
                  <div className="text-xs text-yellow-700 dark:text-yellow-400">
                    ...
                    <p>• Pinjaman minimal {formatCurrency(loanSettings.min_loan_amount)} dan maksimal {formatCurrency(loanSettings.max_loan_amount)}</p>
                  </div>
                </div>
            ...
            >
              ...
              <div className="p-6 space-y-5">
                ...
                <div className="flex gap-3 pt-3">
                  <button
                    onClick={resetSavingTypeModal}
                    className="flex-1 px-4 py-2.5 bg-gray-100 dark:bg-neutral-700 text-gray-700 dark:text-gray-300 font-medium rounded-xl hover:bg-gray-200 transition-colors"
                  >
                    Batal
                  </button>
                  <button
                    ...
                    disabled={isLoading}
                    className="flex-1 px-4 py-2.5 bg-imigrasi-primary text-white font-bold rounded-xl hover:bg-blue-800 transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
                  >
